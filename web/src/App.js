...
const fetchTotalCount = async (searchPhrase, offset) => {
  let totalCountResponse = await fetch(`http://localhost:8000/v1/search/${searchPhrase}?totalcount=1`); //Fetch total count of search results if intial fetch (offset =0)
...
const fetchMovieData = async (searchPhrase, offset, limit) => {
  let movieDataResponse = await fetch(`http://localhost:8000/v1/search/${searchPhrase}?offset=${offset}&limit=${limit}`); //Fetch search results based on offset and limit
...
const App = () => {
  ...
  const handleMovieData = async () => {
    ...
    try {
      ...
      if (searchPhrase.length < 2) {
        throw new Error('Please enter atleast two chacters...')
      }
      if (offset === 0) {
  ...
  return (
    <div className="App">
      ...
      <input type="text" placeholder="Enter search phrase..." onChange={
        event => {
